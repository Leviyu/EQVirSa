...
cout << "---> Read in CRUST 1.0 model" << endl;
	...
	{
		current_file_name = "xyz-bd"+std::to_string(layer_num);
		...
		current_file_name = "xyz-vs"+std::to_string(layer_num);
		myfile.open(current_file_name.c_str());
		for(i = 0 ; i < 180*360 ; i++ )
			myfile >> tmp1>>tmp2>>this->vs[layer_num-1][i];
		myfile.close();

		current_file_name = "xyz-vp"+std::to_string(layer_num);
		myfile.open(current_file_name.c_str());
		for(i = 0 ; i < 180*360 ; i++ )
			myfile >> tmp1>>tmp2>>this->vp[layer_num-1][i];
		myfile.close();

		current_file_name = "xyz-ro"+std::to_string(layer_num);
		myfile.open(current_file_name.c_str());
		for(i = 0 ; i < 180*360 ; i++ )
