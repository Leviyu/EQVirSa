...
std::sort(this->existing_record.begin(), this->existing_record.end());
...
void big_new_record::output_VS_info_for_one_VS(int vs_index)
{
	...
	*/
	for(count = 0; count < my_vs.record_tag_index;count++)
	{
		...
		double record_dist = my_record[sta_index].DIST;
		double SNR = my_record[sta_index].SNR;
		out<< long_file<<" "<< record_dist<< " "<< SNR << "   " <<  my_vs.grid_dist - record_dist << endl;
	}
	out.close();
	...
	<< setw(15) << EQ_NAME
	<< setw(10) << setprecision(3) << my_vs.eq_lat
	<< setw(10) << setprecision(3) << my_vs.eq_lon
	<< setw(10) << setprecision(3) << eq_radius
	<< setw(10) << setprecision(3) << my_vs.sta_lat
	<< setw(10) << setprecision(3) << my_vs.sta_lon
	<< setw(10) << setprecision(3) << sta_radius
	<< setw(10) << setprecision(3) << record_tag_index
	<< setw(10) << setprecision(3) << my_vs.grid_dist
	<< setw(10) << setprecision(3) << my_vs.virtual_stack_ONSET
	<< setw(10) << setprecision(3) << my_vs.tstar_ccc
	<< setw(10) << setprecision(3) << my_vs.tstar_factor
	<< setw(10) << setprecision(3) << my_vs.gau_ccc
	<< setw(10) << setprecision(3) << my_vs.gau_factor
	<< setw(10) << setprecision(3) << my_vs.quality_flag
	<< setw(10) << setprecision(3) << my_vs.ave_SNR
	<< setw(10) << setprecision(3) << my_vs.stack_SNR
	<< setw(10) << setprecision(3) << my_vs.stack_SNR_peak
	<< setw(10) << setprecision(3) << my_vs.misfit
	<< setw(10) << setprecision(3) << my_vs.misfit_pre
	<< setw(10) << setprecision(3) << my_vs.misfit_bak
	<< setw(10) << setprecision(3) << my_vs.stretch_ccc
	<< setw(10) << setprecision(3) << my_vs.stretch_coeff
	<< setw(10) << setprecision(3) << my_vs.polarity
	<< setw(10) << setprecision(3) << my_vs.eq_dep
	<< setw(10) << setprecision(3) << my_vs.PREM
	<< setw(10)  << my_vs.PHASE
	<< setw(10) << setprecision(3) << my_vs.virtual_stack_ONSET
	<< setw(10) << setprecision(3) << my_vs.virtual_stack_ENDSET
	<< setw(10) << setprecision(3) << my_vs.one_period
	<< setw(10) << setprecision(2) << my_vs.stack_SNR_peak_trough
	<< setw(10) << setprecision(0) << my_vs.traffic_phase_nearby
...
void big_new_record::read_INFILE()
{
	...
	while(getline(myfile,tmp))
	{
		...
		flag = "<LAT_EQ_LOC>";
		if(sub1.compare(flag) == 0)
			this->LAT_EQ_LOC= atoi(sub2.c_str());

		flag = "<LON_EQ_LOC>";
		if(sub1.compare(flag) == 0)
			this->LON_EQ_LOC= atoi(sub2.c_str());

		//flag = "<LAT_STA_LOC>";
		//if(sub1.compare(flag) == 0)
			//this->LAT_STA_LOC= atof(sub2.c_str());

		//flag = "<LON_STA_LOC>";
		//if(sub1.compare(flag) == 0)
			//this->LON_STA_LOC= atof(sub2.c_str());

		flag = "<INDEX_LOC>";
		if(sub1.compare(flag) == 0)
			this->INDEX_LOC= atoi(sub2.c_str());

		//cout << " INDEX LOC is "<< this->INDEX_LOC
			//<< " LAT EQ "<< this->LAT_EQ_LOC
			//<<" LON EQ "<< this->LON_EQ_LOC
			//<< endl;
...
void big_new_record::count_record_existance_for_grid_pair()
{
	...
	*/
	...
	this->my_vs_index = this->INDEX_LOC;
	for(ilat_eq = 0; ilat_eq<this->grid_lat_num; ilat_eq++)
	{
		...
		for(ilon_eq = 0; ilon_eq< this->grid_lon_num[ilat_eq]; ilon_eq++)
		{
			...
			if( ilat_eq < this->LAT_EQ_LOC )
				continue;
			if( ilat_eq == this->LAT_EQ_LOC and ilon_eq < this->LON_EQ_LOC )
				continue;
		...
		cout << "EQ_GRID_LAT_LON: "<< ilat_eq << " "<< ilon_eq<< endl;
...
void big_new_record::make_virtual_station_for_EQ(int ilat_eq, int ilon_eq, int ilat_sta, int ilon_sta, int ieq_index)
{
	...
	{
		//cout << " searching for "<< this->my_grid[ilat_sta][ilon_sta].CU_STA_NAME[ista] << endl;
		...
		if(ista_eventStation <= 0) 
			continue;
		//cout << " --> Work on "<< ista << " / " << sta_num << " eventStation index is "<< ista_eventStation << " total sta num  " << istack<<endl;
		//cout << " EQ /STA we find "<< EQ_NAME << " "<< STA_NAME << endl;
		//cout << " what we find "<< this->my_record[ista_eventStation].EQ << " "
			//<< this->my_record[ista_eventStation].STA << endl;
		//double grid_dist = this->my_vs[vs_index].grid_dist ;
		double grid_lat = this->my_vs[vs_index].sta_lat ;
		double grid_lon = this->my_vs[vs_index].sta_lon ;
		//double eq_lat = this->my_vs[vs_index].eq_lat;
		//double eq_lon = this->my_vs[vs_index].eq_lon;
		//double eq_dep = this->my_vs[vs_index].eq_dep;
		double sta_lat = this->my_record[ista_eventStation].sta_lat;
		double sta_lon = this->my_record[ista_eventStation].sta_lon;
		double dist = dist_A_B(grid_lat ,grid_lon, sta_lat , sta_lon);
		//double dif = dist - grid_dist ;
		//cout << " VS dist "<< grid_dist << " sta dist "<< dist <<" diff "<< dif << endl;
			//this->my_record[ista_eventStation].sta_lon << endl;
		//cout << " record - grid center distance "<< dist << endl;
		// if current record is too far away from dist bin, we skip it
		if( dist > 500 ) 
			continue;
		//cout << "grid sta lat lon "<< grid_lat << " "<< grid_lon << endl;
		//cout << "record lat lon "<< sta_lat << " "<< sta_lon << endl;
		//cout << " current distance is "<< dist << endl;
		...
		this->my_vs[vs_index].record_tag_index ++;
	}
	...
	if( this->my_vs_index > 1 and this->my_vs_index != this->INDEX_LOC )
	{
		this->my_vs[this->my_vs_index -1].destruct();
	}
}
...
int big_new_record::individual_VS_processing()
{
	...
	cout << "--> Working on ivs " << vs_index << " total index num: " 
		<< this->my_vs[vs_index].record_tag_index << endl;
	for(count = 0; count < this->my_vs[vs_index].record_tag_index; count ++)
	{
		station_index = this->my_vs[vs_index].record_tag[count];
	...
	this->my_vs[vs_index].PHASE = this->PHASE;
	...
	if(flag == 1)
	{
		cout << " find stack ONSET ERROR " << endl;
		return 1;
	}
	int flag_record_bad = this->my_vs[vs_index].output_stacked_record();
	if ( flag_record_bad == -1)
	{
		cout << " ERROR Warning records is flat, should be rejected!"<< endl;
		return 1;
	}
	...
	else
	{
		for(ieq = 0; ieq<ex_eq_num; ieq ++)
			for(ista = 0; ista < ex_sta_num;ista++)
			{
				...
				//double sta_lat = this->my_record[num].sta_lat;
				//double sta_lon = this->my_record[num].sta_lon;
				//double grid_lat = this->my
...
int big_new_record::find_EQ_STA_PHASE_number_in_eventinfo(string EQ_NAME, string STA, string PHASE)
{
	...
	*/
	...
	binary_loc = this->binary_find(this->existing_record.begin(), this->existing_record.end(),tag[0]);
	/*
if( return_num == 0)
	return binary_loc;
...
int big_new_record::find_EQ_STA_PHASE_number_in_eventStation(string EQ_NAME, string STA)
{
	...
	*/
	...
	tag[0].EQ = EQ_NAME;
	tag[0].STA = STA;
	tag[0].PHASE = this->PHASE;

	//this->binary_find(this->my_record.begin(), this->my_record.end(),tag[0]);
	int binary_loc = 0;
	binary_loc =  this->binary_find(this->my_record.begin(), this->my_record.end(),tag[0]) ;
...
template<class AAA>
int big_new_record::binary_find(AAA begin, AAA end, new_record val)
{
...
void big_new_record::catagorize_eventstation_to_VS()
{
	...
	for(count = 0; count < STA_NUM; count++)
	{
		...
		for(ilat = 0; ilat < this->grid_lat_num ; ilat++)
			for(ilon = 0; ilon < this->grid_lon_num[ilat] ; ilon++)
			{
				...
				if( distance_km > 110*this->VS_STA_RADIUS_DEGREE )
					continue;
				//cout << " station "<< STA_LIST[count] << " is stored in and the distance is " << distance_km /111.1 << endl;
	...
	for(ilat = 0; ilat < this->grid_lat_num ; ilat++)
		for(ilon = 0; ilon < this->grid_lon_num[ilat] ; ilon++)
		{
			...
			{
				myfile2<< "GRID_LAT_LON: "<< ilat << " " << ilon << " lat_lon" << 
				this->my_grid[ilat][ilon].grid_lat<< " " << this->my_grid[ilat][ilon].grid_lon <<
				" GRID_EQ_NUM/STA_NUM: " << this->my_grid[ilat][ilon].CU_EQ_NUM << " "
					 << this->my_grid[ilat][ilon].CU_STA_NUM<< endl;
			}
		}
	myfile2.close();
}
