...
class new_record
{
	public:
		...
		bool operator<(const new_record &rhs){
			...
			else if( this->STA != rhs.STA) 	return this->STA<rhs.STA;
			else if( this->PHASE != rhs.PHASE ) return this->PHASE < rhs.PHASE;
		}
		bool operator!=(const new_record &rhs){
...
//
		...
		void sort_my_record();
		template<class AAA>
		int binary_find(AAA begin, AAA end, new_record val);
		template<class AAA>
...
class virtual_station : public new_record
{
	public:
		...
		double stack_SNR_peak_trough;
		string S_ES_file;
		int find_stack_ONSET_time();
		double virtual_stack_ONSET;
		double virtual_stack_ENDSET;
		double tstar_ccc;
		double tstar_factor;
		double gau_ccc;
		double gau_factor;
