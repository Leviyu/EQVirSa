...
set OUT = $PLOTDIR/${ID}.${PHASE}.${target_ivs}_virsual_station_stack.ps
set OUT_pdf = $PLOTDIR/${ID}.${PHASE}.${target_ivs}_virsual_station_stack.pdf
cat /dev/null >! $OUT
...
set NUM = 1
#set target_ivs = `echo "$target_ivs - 1"|bc`
echo "=====> Working on c01 stack for virsual station $target_ivs /$NUM_MAX"
set TMP = `cat $grid_info|awk -v dd=$target_ivs '$1==dd {print $0}'`
set vs_index = $TMP[1]
set EQ_name = $TMP[2]
set eq_lat = $TMP[3]
set eq_lon = $TMP[4]
...
while($irecord <= $irecord_max)
	...
	echo "$irecord / $irecord_max dist $record_dist"
